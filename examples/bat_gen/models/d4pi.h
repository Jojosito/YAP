...
inline std::unique_ptr<Model> d4pi_model()
{
    ...
    // a_1
    auto a_1 = F.resonance(F.pdgCode("a_1+"), radialSize, std::make_shared<RelativisticBreitWigner>());
...
inline d4pi_fit_fitFraction()
{
    fit_fitFraction m("D4pi_frac_fit", d4pi_model());

    auto M = m.model();
    auto D = lone_elt(M.initialStateParticles());
